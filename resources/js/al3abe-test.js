...
if (root) {
    ...
    function renderCategories() {
        ...
        state.categories.forEach((category) => {
            ...
            card.className = `group rounded-2xl border p-4 text-right transition ${isSelected
                ? 'border-amber-400 bg-amber-400/15 ring-2 ring-amber-400/40'
                : 'border-white/10 bg-slate-800/70 hover:border-sky-300/60 hover:bg-slate-800'}`;
            card.innerHTML = `
                <div class="mb-3 flex items-start justify-between gap-3">
                    <span class="grid size-10 place-items-center rounded-xl bg-sky-400/15 text-xl">🎯</span>
                    <span class="rounded-full px-2 py-1 text-xs font-bold ${isSelected ? 'bg-amber-400 text-slate-950' : 'bg-white/10 text-slate-300'}">${isSelected ? 'مختارة' : 'اختيار'}</span>
                </div>
                <h3 class="font-bold text-white">${escapeHtml(category.name)}</h3>
                <p class="mt-2 text-sm leading-6 text-slate-400">${escapeHtml(category.description ?? 'لا يوجد وصف لهذه الفئة.')}</p>
            `;
    ...
    function renderHints(hints) {
        ...
        [...hints]
            ...
            .forEach((hint) => {
                ...
                card.className = `rounded-2xl border p-4 ${style.classes}`;
                card.innerHTML = `
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <h3 class="font-bold text-white">${style.title}</h3>
                            <p class="mt-1 text-xs text-slate-300">${style.subtitle}</p>
                        </div>
                        <button type="button" data-reveal-hint="${hint.level_id}" class="rounded-xl px-3 py-2 text-sm font-bold text-white transition ${style.button}">إظهار</button>
                    </div>
                    <p data-hint-text="${hint.level_id}" class="mt-4 hidden leading-7 text-slate-100"></p>
                `;
        ...
        if (hints.length === 0) {
            elements.hints.innerHTML = '<p class="rounded-xl bg-slate-800 p-4 text-sm text-slate-300">لم يُرجع الـ API أي تلميحات لهذا السؤال.</p>';
        }
    }
